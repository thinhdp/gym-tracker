    ...
    <div className="mt-4 space-y-3">
      ...
      {workouts.map((w) => (
        <div key={w.id} className="rounded-2xl border">
          ...
          {expandedId === w.id && (
            <div className="space-y-3 p-4 border-top">
                                ...
                                onNumberChange={(v) =>
                                  updateWorkout(w.id, {
                                    exercises: w.exercises.map((e2, i2) =>
                                      i2 === idx
                                        ? { ...e2, sets: e2.sets.map((ss, j) => (j === sidx ? { ...ss, weight: fromDisplayWeight(v, unit) } : ss)) }
                                        : e2
                                    ),
                                  })
                                }
                              ...
                              <NumberInputAutoClear
                                ...
                                onNumberChange={(v) =>
                                  updateWorkout(w.id, {
                                    exercises: w.exercises.map((e2, i2) =>
                                      i2 === idx
                                        ? { ...e2, sets: e2.sets.map((ss, j) => (j === sidx ? { ...ss, reps: v } : ss)) }
                                        : e2
                                    ),
                                  })
                                }
                      ...
                      {we.sets.length < 5 && (
                        <Button
                          ...
                          onClick={() =>
                            updateWorkout(w.id, {
                              exercises: w.exercises.map((e2, i2) =>
                                i2 === idx ? { ...e2, sets: [...e2.sets, { set: e2.sets.length + 1, weight: 0, reps: 0 }] } : e2
                              ),
                            })
                          }
                        >
