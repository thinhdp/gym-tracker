    ...
    <div className="rounded-2xl border p-3">
      <div className="mb-2 flex items-center justify-between">
        <div className="font-medium">
          {item.exerciseName}{recommendRep ? <span className="ml-2 text-xs text-neutral-500">({recommendRep})</span>:null}
        </div>
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2">
                <NumberInputAutoClear step="0.5" min="0" className="w-24"
                  ...
                  onNumberChange={(v)=> setSets((prev)=> prev.map((p,i)=> i===idx ? { ...p, weight: fromDisplayWeight(v, unit) } : p))}/>
                <span className="text-xs text-neutral-500">{unit}</span>
              </div>
              <div className="flex items-center gap-2">
                <NumberInputAutoClear step="1" min="0" className="w-24"
                  valueNumber={s.reps}
