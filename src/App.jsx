...
export default function App() {
  ...
  useEffect(()=> {
    ...
    setExercises(prev=> prev.map(e=> ({
      ...e, lastWorkout: latestByName[e.name] ? { date: latestByName[e.name].date, sets: latestByName[e.name].sets } : null
    })));
    ...
    <div className="min-h-screen w-full bg-neutral-50 text-neutral-900">
      <div className="mx-auto max-w-md p-4 pb-24">
        <header className="sticky top-0 z-10 bg-neutral-50">
          <div className="flex items-center justify-between py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="secondary" onClick={()=> setUnit(u=> u==="kg" ? "lb" : "kg")}>Unit: {unit.toUpperCase()}</Button>
            </div>
          </div>
          ...
          <div className="flex justify-end mb-2">
            <ImportExportControls exercises={exercises} workouts={workouts} setExercises={setExercises} setWorkouts={setWorkouts} />
          </div>
        </header>
        ...
        <main className="mt-2">
          {tab==="workouts" && (
            <>
              <WorkoutPlanner exercises={exercises} setExercises={setExercises} workouts={workouts} setWorkouts={setWorkouts} unit={unit} onCreated={()=>setTab("workouts")} />
              <WorkoutHistory workouts={workouts} setWorkouts={setWorkouts} exercises={exercises} setExercises={setExercises} unit={unit} />
            </>
          )}
