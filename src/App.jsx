...
function Button({ children, className, ...props }) {
  return (
    <button
      className={`px-3 py-1.5 text-sm border rounded-xl hover:bg-gray-50 active:scale-[0.99] ${className || ""}`}
      {...props}
    >
      {children}
    </button>
...
function normalizeExercise(ex) {
  ...
  if (!name) return null;
  return { name, recommendRep, lastWorkout: null }; // lastWorkout will be recomputed
}
...
function normalizeWorkout(w) {
  ...
  const normExercises = exercises.map(e => {
...
function mergeExercises(current, incoming) {
  ...
  for (const inc of incoming) {
    ...
    if (!map.has(key)) {
      ...
      // If current has no recommendation but incoming does, adopt it.
...
function mergeWorkouts(current, incoming) {
  ...
  for (const w of incoming) {
    if (!ids.has(w.id)) {
      ...
      // Collision: give new id to imported workout
...
function ImportExportControls({ exercises, workouts, setExercises, setWorkouts }) {
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <Button onClick={onExport}><Save/> Export</Button>
      <input ref={fileRef} type="file" accept="application/json" className="hidden" onChange={onFileChange}/>
      <Button onClick={() => onImportClick("merge")}>Import (merge)</Button>
      <Button onClick={() => onImportClick("replace")}>Import (replace)</Button>
    </div>
  );
}
        ...
        <header className="sticky top-0 z-10 bg-neutral-50">
          <div className="flex items-center justify-between py-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button onClick={() => setUnit(u => (u === "kg" ? "lb" : "kg"))}>Unit: {unit.toUpperCase()}</Button>
            </div>
          </div>
          <div className="flex gap-2 mb-2">
            <Button className={tab === "workouts" ? "bg-black text-white" : ""} onClick={() => setTab("workouts")}>Workouts</Button>
            <Button className={tab === "calendar" ? "bg-black text-white" : ""} onClick={() => setTab("calendar")}>Calendar</Button>
            <Button className={tab === "exercises" ? "bg-black text-white" : ""} onClick={() => setTab("exercises")}>Exercises</Button>
          </div>
  ...
  return (
    <div className="mt-3 grid gap-2 rounded-xl border p-3">
      ...
      <div className="flex justify-end gap-2">
        <Button onClick={() => { setName(""); setRep(""); }}>Reset</Button>
        <Button onClick={handleSave} disabled={!canSave}><Save/> Save</Button>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-2xl border p-3">
      <div className="flex items-center justify-between">
        <div>
          ...
          {!editing ? (
            <p className="text-sm text-neutral-600">{ex.recommendRep || <span className="italic text-neutral-400">No recommendation</span>}</p>
          ) : (
        ...
        <div className="flex items-center gap-2">
          {!editing ? (
            ...
            <>
              <Button onClick={() => { onUpdate(ex.name, { recommendRep: rep }); setEditing(false); }}><Check/></Button>
              <Button onClick={() => { setRep(ex.recommendRep || ""); setEditing(false); }}><X/></Button>
            </>
          )}
          <Button onClick={() => onDelete(ex.name)}><Trash2/></Button>
        </div>
      </div>
      <div className="mt-2">
        ...
        {ex.lastWorkout ? (
          <div className="mt-1 flex flex-wrap items-center gap-2 text-sm">
            <Badge><CalendarIcon/> {fmtDate(ex.lastWorkout.date)}</Badge>
  ...
  const addExerciseByName = (rawName) => {
    ...
    if (!items.some(i => i.exerciseName.toLowerCase() === n.toLowerCase())) {
      ...
      if (exists?.lastWorkout?.sets?.length) {
        ...
        initSets = [{ set: 1, weight: lastSet.weight || 0, reps: lastSet.reps || 0 }]; // stored in kg
      }
    ...
    <Card className="mb-4">
      <CardContent>
        ...
        <div className="grid gap-3">
          ...
          <div className="flex justify-end gap-2">
            <Button onClick={() => { setItems([]); setName(""); setDate(todayStr()); }}>Reset</Button>
            <Button onClick={saveWorkout} disabled={items.length === 0}><Save/> Save Workout</Button>
          </div>
        </div>
      </CardContent>
    </Card>
  ...
  return (
    <div className="relative" ref={ref}>
      <div className="flex items-center gap-2">
        ...
        />
        <Button onClick={() => choose(val || "")}><ListPlus/> Add</Button>
      </div>
      {open && options.length > 0 && (
        <div className="absolute z-20 mt-1 w-full overflow-hidden rounded-xl border bg-white shadow">
          {options.map((o) => (
            <button key={o} onClick={() => choose(o)} className="flex w-full items-center justify-between px-3 py-2 text-left text-sm hover:bg-neutral-50">
              <span>{o}</span>
              <ChevronDown/>
            </button>
          ))}
...
function WorkoutExerciseEditor({ item, onChange, onRemove, unit, recommendRep }) {
  ...
  const addSet = () => setSets(prev => prev.length >= 5 ? prev : [...prev, { set: prev.length + 1, weight: 0, reps: 0 }]);
  const delSet = (i) => setSets(prev => prev.filter((_, idx) => idx !== i).map((s, idx) => ({ ...s, set: idx + 1 })));
  ...
  return (
    <div className="rounded-2xl border p-3">
      <div className="mb-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <Button onClick={addSet} disabled={sets.length >= 5}><Plus/> Set</Button>
            ...
            <div className="flex-1 grid grid-cols-2 gap-3">
              <div className="flex items-center gap-2">
                <Input
                  ...
                  min="0"
                  className="w-24"
                  value={toDisplayWeight(s.weight, unit)}
                  onChange={e => {
                    const v = Number(e.target.value);
                    if (!Number.isFinite(v)) return;
                    const kg = fromDisplayWeight(v, unit);
                    setSets(prev => prev.map((p, i) => i === idx ? { ...p, weight: kg } : p));
                  }}
                />
              ...
              <div className="flex items-center gap-2">
                <Input
                  ...
                  onChange={e => {
                    const v = Number(e.target.value);
                    if (!Number.isFinite(v)) return;
                    setSets(prev => prev.map((p, i) => i === idx ? { ...p, reps: v } : p));
                  }}
                />
                <span className="text-xs text-neutral-500">reps</span>
              </div>
            </div>
            <Button onClick={() => delSet(idx)} disabled={sets.length <= 1}><Trash2/></Button>
          </div>
        ))}
      </div>
    </div>
...
function WorkoutHistory({ workouts, setWorkouts, exercises, setExercises, unit }) {
  ...
  const addExerciseToWorkout = (workout, exerciseName) => {
    ...
    // Prefill from last set if available (kg internally)
      ...
      {workouts.map(w => (
        <div key={w.id} className="rounded-2xl border">
          <div className="flex items-center justify-between px-4 py-3">
            ...
            <div className="flex items-center gap-2">
              <Button onClick={() => setExpandedId(prev => prev === w.id ? null : w.id)}>
                Details <ChevronDown open={expandedId === w.id}/>
              </Button>
              <Button onClick={() => deleteWorkout(w.id)}><Trash2/></Button>
            </div>
          </div>
          ...
          {expandedId === w.id && (
            <div className="space-y-3 border-t p-4">
                  ...
                  <div key={idx} className="rounded-xl border p-3">
                    <div className="mb-2 flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button onClick={() => updateWorkout(w.id, { exercises: w.exercises.filter((_, i2) => i2 !== idx) })}><Trash2/></Button>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-2">
                      {we.sets.map((s, sidx) => (
                        <div key={sidx} className="flex items-center gap-3 rounded-xl border px-3 py-2">
                              ...
                              <Input
                                ...
                                onChange={e => updateWorkout(w.id, {
                                  exercises: w.exercises.map((e2, i2) => i2 === idx ? {
                                    ...e2,
                                    sets: e2.sets.map((ss, j) => j === sidx ? { ...ss, weight: fromDisplayWeight(Number(e.target.value), unit) } : ss)
                                  } : e2)
                                })}
                              />
                              <span className="text-xs text-neutral-500">{unit}</span>
                            ...
                            <div className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={e => updateWorkout(w.id, {
                                  exercises: w.exercises.map((e2, i2) => i2 === idx ? {
                                    ...e2,
                                    sets: e2.sets.map((ss, j) => j === sidx ? { ...ss, reps: Number(e.target.value) } : ss)
                                  } : e2)
                                })}
                              />
                              <span className="text-xs text-neutral-500">reps</span>
                          ...
                          <Button disabled={we.sets.length <= 1}
                      ...
                      {we.sets.length < 5 && (
                        <Button className="w-fit" onClick={() => updateWorkout(w.id, {
                          exercises: w.exercises.map((e2, i2) => i2 === idx ? {
                            ...e2,
                            sets: [...e2.sets, { set: e2.sets.length + 1, weight: 0, reps: 0 }]
                          } : e2)
                        })}><Plus/> Add set</Button>
                      )}
                    </div>
              ...
              <div className="flex justify-end">
                <Button onClick={() => updateWorkout(w.id, { ...w })}><Save/> Update</Button>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-7 gap-1">
            {days.map((ds, idx) => {
              ...
              return (
                <button
                  key={idx}
                  className={`aspect-square rounded-xl border text-sm flex flex-col items-center justify-center ${isSelected ? "bg-black text-white" : "bg-white"}`}
      ...
      <Card>
        <CardContent>
          <div className="flex items-center justify-between mb-2">
            ...
            <Button onClick={createWorkoutForSelected}><Plus/> New workout</Button>
          </div>
